...
namespace App\Http\Controllers\users\qbank_user\qbank_last_marked_fetch_question;
...
class QbankLastMarkedFetchedQuestionController extends Controller
{
    public function fetchMarkedQuestions(Request $request)
    {
    ...
    $correctQuestions = QbankMarked::where('qbank_id', $qbankId)
        ->where('id', $user_id)
        ->get();

    // Get the last fetched question for each system and user
    $lastCorrectFetchedQuestions = LastMarkedFetchedQuestion::whereIn('qbank_system_id', $systemIds)
    ...
    foreach ($systemIds as $systemId) {
        ...
        if ($systemQuestions->isNotEmpty()) {
            ...
            $lastFetchedQuestion = LastMarkedFetchedQuestion::where('qbank_system_id', $systemId)
                ->where('qbank_id', $qbankId)
                ->where('user_id', $user_id)
                ->first();
            ...
            if ($lastFetchedQuestion) {
                ...
                LastMarkedFetchedQuestion::create([
    ...
    if ($remainingQuestions > 0) {
        ...
        $additionalQuestions = QbankMarked::where('qbank_id', $qbankId)
            ->where('id', $user_id)
