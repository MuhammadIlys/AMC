...
namespace App\Http\Controllers\users\qbank_user\qbank_last_correct_fetch_question;
...
class QbankLastCorrectFetchedQuestionController extends Controller
{

    public function fetchCorrectQuestions(Request $request)
{
    ...
    $correctQuestions = QbankCorrects::where('qbank_id', $qbankId)
        ->where('id', $user_id)
        ->get();

    // Get the last fetched question for each system and user
    $lastCorrectFetchedQuestions = LastCorrectFetchedQuestion::whereIn('qbank_system_id', $systemIds)
    ...
    foreach ($systemIds as $systemId) {
        ...
        if ($systemQuestions->isNotEmpty()) {
            ...
            $lastFetchedQuestion = LastCorrectFetchedQuestion::where('qbank_system_id', $systemId)
                ->where('qbank_id', $qbankId)
                ->where('user_id', $user_id)
                ->first();
            ...
            if ($lastFetchedQuestion) {
                ...
                LastCorrectFetchedQuestion::create([
    ...
    if ($remainingQuestions > 0) {
        ...
        $additionalQuestions = QbankCorrects::where('qbank_id', $qbankId)
            ->where('id', $user_id)
