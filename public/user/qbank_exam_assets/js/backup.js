...
function onMonthClick2(year, month, solve) {
  ...
  $.ajax({
    ...
    method: "POST",
    data: { year: year, month: month, solve: solve },
    dataType: "json",
...
function loadQuestionByIndex(index) {
  ...
  if (index >= 0 && index < questionsData.length) {
    ...
    $('#question-explanation').html( question.question_explination);
...
document.addEventListener('keydown', function(event) {
  if (event.ctrlKey && (event.key === 'c' || event.key === 'C')) {
    ...
    showErrorAlert("you are vaiolating our terms and condition");
